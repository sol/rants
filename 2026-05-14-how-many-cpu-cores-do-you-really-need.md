...
**NB:** For GHC builds we are hitting a hard limit here.  For `cabal`, however, this is highly workload dependent.  See my [final remarks](#final-remarks) for why this is.
...
Hyperthreading doesn't contribute anything.  Given that we didn't see scaling beyond ~6 cores, this makes perfect sense.  On the upside, at least performance didn't degrade massively with increasing concurrency.
...
It has:
- 8 performance cores
- according to synthetic benchmarks, the highest single core performance of any x86-64 CPU (alongside the 285K)
...
>
> It may very well be that for Haskell development the ***270K*** does not offer any noticeable advantage over the ***9700X*** (see next section).  This is the whole point I'm trying to make with [`ghc-bench`][ghc-bench]: We should base purchasing decisions on real world performance, not synthetic benchmarks.
